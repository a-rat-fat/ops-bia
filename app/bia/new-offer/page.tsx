...
export default function NewOffer() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/offers', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ...form, meters: Number(form.meters||0), agencyId: Number(form.agencyId) }) })
    if (res.ok) r.push('/bia'); else setSaving(false)
  }
  return (
    <div className="card max-w-xl">
      <h1 className="text-xl font-semibold mb-3">Nouvelle OFFRE</h1>
